...
/**
 * A Hash Table implementation - Seperate Chaining.
 *
...
public class HashTable_SeperateChaining<K, V> implements Iterable<K> {
    ...
    public HashTable_SeperateChaining() {
        this(default_capacity, default_load_factor);
    }

    public HashTable_SeperateChaining(int capacity) {
        this(capacity, default_load_factor);
    }

    public HashTable_SeperateChaining(int capacity, double maxLoadFactor) {
        if (capacity < 0)
    ...
    private V bucketInsertEntry(int bucketIndex, Entry<K, V> entry) {
        LinkedList<Entry<K, V>> bucket = table[bucketIndex];
        // If bucket does not exist create a new bucket as an empty linked list and add
        // to the table
        if (bucket == null)
        ...
        // Otherwise overwrite existing entry
    ...
    private V bucketRemoveEntry(int bucketIndex, K key) {
        Entry<K, V> entry = bucketSeekEntry(bucketIndex, key);
        if (entry != null) {
            LinkedList<Entry<K, V>> chain = table[bucketIndex];
            chain.remove(entry);
            --size;
            return entry.value;
        }
